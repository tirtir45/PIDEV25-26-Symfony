...
class EvaluationAutomatiqueService
{
    private const CRITERES = [
        'qualite_description' => ['poids' => 15, 'max_score' => 15],
        'objectifs_clairs'    => ['poids' => 15, 'max_score' => 15],
        'budget_realiste'     => ['poids' => 20, 'max_score' => 20],
        'duree_realiste'      => ['poids' => 10, 'max_score' => 10],
        'equipe_adequate'     => ['poids' => 10, 'max_score' => 10],
        'secteur_pertinent'   => ['poids' => 10, 'max_score' => 10],
        'financement'         => ['poids' => 10, 'max_score' => 10],
        'experiences'         => ['poids' => 10, 'max_score' => 10],
    ];

    private const SEUIL_ACCEPTATION = 70;
    ...
    {
        $longueur = strlen($projet->getDescription() ?? '');

        if ($longueur >= 500)      { $score = 15; $niveau = 'excellent'; $commentaire = 'Description très complète et détaillée'; }
        elseif ($longueur >= 300)  { $score = 12; $niveau = 'bon';       $commentaire = 'Bonne description, pourrait être plus détaillée'; }
        elseif ($longueur >= 150)  { $score = 8;  $niveau = 'correct';   $commentaire = 'Description correcte mais manque de détails'; }
        elseif ($longueur >= 50)   { $score = 4;  $niveau = 'insuffisant'; $commentaire = 'Description trop courte'; }
        else                       { $score = 0;  $niveau = 'critique';  $commentaire = 'Description très insuffisante'; }

        return ['score' => $score, 'max_score' => 15, 'niveau' => $niveau, 'commentaire' => $commentaire,
    ...
    private function evaluerObjectifsClairs(Projets $projet): array
    {
        ...
        foreach ($mots_cles as $mot) {
            if (stripos($objectifs, $mot) !== false) $score_mots_cles += 2;
        }
        ...
        return ['score' => $score, 'max_score' => 15,
            'niveau' => $score >= 12 ? 'bon' : ($score >= 8 ? 'correct' : 'insuffisant'),
            'commentaire' => $commentaire,
            'recommandation' => 'Utilisez la méthode SMART pour définir vos objectifs'];
    }

    private function evaluerBudgetRealiste(Projets $projet): array
    {
        $budget = $projet->getBudgetEstime() ?? 0;

        if ($budget >= 10000 && $budget <= 500000)    { $score = 20; $commentaire = 'Budget très réaliste pour un projet de cette envergure'; }
        elseif ($budget >= 5000 && $budget <= 1000000) { $score = 15; $commentaire = 'Budget acceptable'; }
        elseif ($budget >= 1000 && $budget <= 2000000) { $score = 10; $commentaire = 'Budget à justifier davantage'; }
        elseif ($budget < 1000)                        { $score = 5;  $commentaire = 'Budget trop faible pour être réaliste'; }
        else                                           { $score = 8;  $commentaire = 'Budget très élevé, nécessite plus de justifications'; }
    ...
    {
        $duree = $projet->getDureeEstimee() ?? 0;
        $budget = $projet->getBudgetEstime() ?? 0;

        if ($duree >= 3 && $duree <= 18)      { $score = 10; $commentaire = 'Durée réaliste'; }
        elseif ($duree >= 1 && $duree <= 24)  { $score = 7;  $commentaire = 'Durée acceptable'; }
        elseif ($duree < 1)                   { $score = 3;  $commentaire = 'Durée trop courte'; }
        else                                  { $score = 5;  $commentaire = 'Durée trop longue'; }

        $ratio = $duree > 0 ? $budget / $duree : 0;
        if ($ratio >= 5000 && $ratio <= 50000) {
            $score += 2;
            $commentaire .= ' avec un budget mensuel cohérent';
        }

        return ['score' => min(10, $score), 'max_score' => 10,
    ...
    {
        $nbMembres = $projet->getNbMembresEquipe() ?? 0;
        $duree = $projet->getDureeEstimee() ?? 0;

        if ($nbMembres >= 2 && $nbMembres <= 10)      { $score = 10; $commentaire = 'Taille d\'équipe idéale'; }
        elseif ($nbMembres >= 1 && $nbMembres <= 15)  { $score = 7;  $commentaire = 'Taille d\'équipe acceptable'; }
        else                                          { $score = 4;  $commentaire = 'Taille d\'équipe non adaptée'; }
        ...
        return ['score' => max(0, $score), 'max_score' => 10,
            'niveau' => $score >= 8 ? 'bon' : ($score >= 5 ? 'correct' : 'insuffisant'),
            'commentaire' => $commentaire,
    ...
    {
        $secteurs_prioritaires = ['Technologie', 'Innovation', 'Green Tech', 'Santé', 'Éducation',
        ...
        $score = $estPrioritaire ? 10 : 6;

        return ['score' => $score, 'max_score' => 10,
            'niveau' => $score >= 8 ? 'bon' : 'correct',
            'commentaire' => $estPrioritaire ? 'Secteur prioritaire pour l\'incubation' : 'Secteur standard',

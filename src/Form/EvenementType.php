...
class EvenementType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        // Date constraints: Ajouter requires future date, Modifier only requires not-null (Java logic)
        $dateConstraints = [
            new Assert\NotBlank(message: 'Le champ « Date » est obligatoire.'),
        ];
        if ($isNew) {
        ...
        // Capacité: Ajouter requires > 0, Modifier allows >= 0 (Java logic)
        $capaciteConstraints = [
            new Assert\NotBlank(message: 'Le champ « Capacité » est obligatoire.'),
        ];
        if ($isNew) {
            $capaciteConstraints[] = new Assert\Positive(
                message: 'La capacité doit être supérieure à 0.'
            );
        } else {
            $capaciteConstraints[] = new Assert\PositiveOrZero(
        ...
        $builder
            ...
            ->add('lieu', TextType::class, [
                ...
                'constraints' => [
                    new Assert\NotBlank(message: 'Le champ « Lieu » est obligatoire.'),
                ],
            ])
            ...
            ->add('description', TextareaType::class, [
                ...
                'constraints' => [
                    new Assert\NotBlank(message: 'Le champ « Description » est obligatoire.'),
                ],
            ])

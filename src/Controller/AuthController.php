...
use App\Service\BehaviorAIService;
...
class AuthController extends AbstractController
{
    ...
    public function login(Request $request, UtilisateursRepository $repo, EntityManagerInterface $em, CaptchaService $captcha, BehaviorAIService $behaviorAI): Response
    {
            ...
            if (empty($email) || empty($password)) {
                ...
                if (!$user) {
                    ...
                    if ($passwordValid) {
                        ...
                        $behaviorResult = $behaviorAI->analyze($user, $loginTime);
                        $request->getSession()->set('behavior_anomaly', $behaviorResult);

                        if ($behaviorResult['anomaly'] && $behaviorResult['risk_level'] !== 'low') {
                            $this->addFlash('behavior_warning', $behaviorResult['message']);
                        }
        ...
        if ($request->isMethod('POST')) {
            ...
            if (empty($nom)) {
                ...
                try {
                    ...
                    // Redirect to login page with success message (don't auto-login)
                    $this->addFlash('success', 'Compte créé avec succès ! Connectez-vous pour commencer.');

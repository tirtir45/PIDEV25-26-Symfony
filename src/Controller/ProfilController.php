...
use App\Service\PhotoValidationService;
...
class ProfilController extends AbstractController
{
    ...
    public function index(
        ...
        PhotoValidationService $photoValidator
    ): Response {
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($action === 'delete') {
                $reclamations = $em->getRepository(\App\Entity\Reclamations::class)
            ...
            // ── Validation photo via microservice IA ──
            $photoFile = $request->files->get('photo');
            if ($photoFile) {
                $allowedMimes = ['image/jpeg', 'image/png', 'image/webp'];
                if (!in_array($photoFile->getMimeType(), $allowedMimes, true)) {
                    $errors['photo'] = 'Format non supporté. Utilisez JPEG, PNG ou WebP.';
                } elseif ($photoFile->getSize() > 5 * 1024 * 1024) {
                    $errors['photo'] = 'La photo ne doit pas dépasser 5 Mo.';
                } else {
                    $result = $photoValidator->validate($photoFile);
                    if (!$result['valid']) {
                        $errors['photo'] = $result['message'];
                    }
                }
            }

...
final class CommandesController extends AbstractController
{
    ...
    #[Route('/fournisseur/{idCommande}/borderau', name: 'app_fournisseur_borderaux', methods: ['GET'])]
    public function fournisseurBorderau(int $idCommande, Request $request, Lignes_commandeRepository $lignesCommandeRepository, CommandesRepository $commandesRepository, RessourcesRepository $ressourcesRepository, EntityManagerInterface $entityManager): Response
    {
        $commande = $commandesRepository->find($idCommande);
        if (!$commande) {
            $this->addFlash('error', 'Commande introuvable.');
        ...
        // Security check: Only the Fournisseur owner of at least one resource in this order can print it
        $isFournisseur = ($request->getSession()->get('user_role') === 'Fournisseur');
        $userId = $request->getSession()->get('user_id');
        $user = $userId ? $entityManager->getRepository(Utilisateurs::class)->find($userId) : null;
        
        if (!$isFournisseur || !$user instanceof Utilisateurs) {
        ...
        foreach ($lines as $line) {
            $owner = $line->getId_ressource()->getId_fournisseur();
            $ownerId = $owner instanceof Utilisateurs ? (int) $owner->getId() : (int) $owner;
        ...
        if (!$belongsToFournisseur) {
            $this->addFlash('error', 'Vous n’êtes pas autorisé à imprimer ce bordereau.');
            return $this->redirectToRoute('app_fournisseur_ventes');
        }
        ...
        $html = $this->renderView('commandes/borderau.html.twig', [
            'commande' => $commande,
            'lines' => $lines,
            'resource_names' => $resourceNames,
            'fournisseur' => $user,
            'entrepreneur' => $commande->getIdEntrepreneur()
        ]);
        ...
        return new Response($dompdf->output(), 200, [
            ...
            'Content-Disposition' => 'inline; filename="borderau-commande-'.$commande->getIdCommande().'.pdf"',
        ]);
    }
